...
// Highlight selected checkboxes on load
...
// Change background on check/uncheck + GROUP SELECT
routeCheckboxes.on('change', function(){
    ...
    // Agar route '/*' ya '/controller/*' type ka hai
    var routeVal = _t.val(); // example: /assessment/*

    if (routeVal.slice(-2) === '/*') {
        // prefix: '/assessment/'  ya '/' (root)
        var prefix = routeVal.slice(0, -1); // remove only '*'

        routeCheckboxes.each(function(){
            var cb = $(this);
            var cbVal = cb.val();

            // same group ke saare routes (including nested)
            if (cbVal.indexOf(prefix) === 0 && cbVal !== routeVal) {
                cb.prop('checked', _t.is(':checked'));

                // background bhi update ho jaye
                if (cb.is(':checked')) {
                    cb.closest('label').css('background', backgroundColor);
                } else {
                    cb.closest('label').css('background', 'none');
                }
            }
        });
    }
