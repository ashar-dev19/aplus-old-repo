...
$redirectUrl = \Yii::$app->urlManager->createUrl(['result-page', 'lesson_id' => $lesson_id]);
?>


<script>
(function() {
  var lessonId = parseInt(<?= json_encode((int) $lesson_id) ?>);
  var quizCompletedKey = "quizCompleted_" + lessonId;
  if (localStorage.getItem(quizCompletedKey) === "true") {
    localStorage.removeItem("solutionModalCount_" + lessonId);
    localStorage.removeItem("tutorialPressCount_" + lessonId);
    localStorage.removeItem("lastVisitedSlide_" + lessonId + "_<?= (int) $studentId ?>");
            ...
            if (currentSlideIndex == totalSlides - 1 && !formSubmitting) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var lessonId = parseInt(" . (int) $lesson_id . ");
                        localStorage.removeItem(\"solutionModalCount_\" + lessonId);
                        localStorage.removeItem(\"tutorialPressCount_\" + lessonId);
                        localStorage.setItem(\"quizCompleted_\" + lessonId, 'true');
...
<script>
    ...
    function updateSlideIndex(currentIndex) {
      ...
      var lessonId = <?= json_encode((int) $lesson_id) ?>;
...
<script>
  ...
  localStorage.removeItem(storageKey);  
  localStorage.removeItem(quizCompletedKey); 
  localStorage.removeItem("solutionModalCount_<?= $lesson_id ?>");
